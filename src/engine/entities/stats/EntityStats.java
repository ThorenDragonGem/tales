...
public class EntityStats
{
	...
	private double nextLevelXp, xpLevel, totalXP;
	...
	public EntityStats(Entity e)
	{
		...
		stats.put("defense", new LevelStats(entity.getRank(), 0f, 0f));
		// strength stat => damages
		stats.put("strength", new LevelStats(entity.getRank(), 1f, 0f));
		// speed stat
		stats.put("velocity", new Stats(1f, 0f));
		// attack range stat
		stats.put("atr", new Stats(Mathf.sqrt(entity.getBody().getBounds().width * entity.getBody().getBounds().width + entity.getBody().getBounds().height * entity.getBody().getBounds().height) / 2, Mathf.sqrt(entity.getBody().getBounds().width * entity.getBody().getBounds().width + entity.getBody().getBounds().height * entity.getBody().getBounds().height) / 2));
		// sight range stat => max distance an entity can look
		stats.put("sgr", new Stats(500f, Mathf.sqrt(entity.getBody().getBounds().width * entity.getBody().getBounds().width + entity.getBody().getBounds().height * entity.getBody().getBounds().height) / 2));
		...
		stats.put("healthMax", new LevelStats(entity.getRank(), 100f, 1f));
		...
		// ticks)
		stats.put("regen", new LevelStats(entity.getRank(), 0f, 0f));
		...
		// magic
		stats.put("power", new LevelStats(entity.getRank(), 0f, 0f));
		stats.put("resistance", new LevelStats(entity.getRank(), 0f, 0f));
		stats.put("mpen", new Stats(0f, 0f, 100f));
		stats.put("manaMax", new LevelStats(entity.getRank(), 100f, 0f));
		stats.put("manaRegen", new LevelStats(entity.getRank(), 0f, 0f));
	...
	public void physicalDamages(double factor)
	{
		if(target.isDefending())
		{
			...
			// shield damages
			if(target.getStats().shield > 0)
			{
				if(damages > target.getStats().shield)
				{
					damages -= target.getStats().shield;
					target.getStats().shield = 0;
				}
				else
				{
					target.getStats().shield -= damages;
					damages = 0;
				}
			}
			// health damages
			target.getStats().health -= damages;
			// spellVamp stuff
	...
	{
		double defense = target.getStats().get("defense");
		double strength = get("strength");
	...
	{
		double resistance = target.getStats().get("resistance");
		double power = get("power");
	...
	public void setXp(double amount)
	{
		...
		while(xpToAdd > nextLevelXp && level <= maxLevel)
		{
